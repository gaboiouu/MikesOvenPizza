...
import { Heart, ArrowLeft, ShoppingCart, Plus, CheckCircle } from 'lucide-react';
...
const Favoritos: React.FC = () => {
  ...
  const addToCart = (pizza: Pizza, size: 'Personal' | 'Grande') => {
    ...
    showToast(`✅ ${pizza.name} (${size}) agregado al carrito`);
  };
  ...
  return (
    <div className="min-h-screen bg-[#F3E3C2]/20 py-8">
        ...
        {favoritos.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {favoritos.map((pizza) => (
              <div
                key={pizza.id}
                className="bg-white rounded-xl overflow-hidden shadow-lg border border-gray-100 hover:shadow-2xl transition group"
              >
                ...
                <div className="p-6">
                  ...
                  <div className="mb-4">
                    <div className="font-bold text-[#D14B4B]">
                    ...
                    {pizza.priceGrande ? (
                      <>
                        <button
                          ...
                          className="flex-1 bg-[#0D4D45] hover:bg-[#08332e] text-white font-bold py-2 rounded-lg text-sm uppercase flex items-center justify-center gap-1 transition"
                        >
                          <Plus size={16} /> Personal
                        </button>
                        <button
                          onClick={() => addToCart(pizza, 'Grande')}
                          className="flex-1 bg-[#D14B4B] hover:bg-red-700 text-white font-bold py-2 rounded-lg text-sm uppercase flex items-center justify-center gap-1 transition"
                        >
                          <Plus size={16} /> Grande
                        </button>
                      </>
      ...
      {toast && (
        <div className="fixed bottom-6 right-6 bg-[#0D4D45] text-white px-5 py-3 rounded-xl shadow-2xl flex items-center gap-2 animate-bounce">
          <CheckCircle size={20} />
